...
import { updateRecurringPaymentStatusById } from "../database/supabase";

const errorDecoder = ErrorDecoder.create();

async function cancelRecurringPayment(id: number, nonce?: number) {
    try {
        ...
        console.log("Cancel recurring payment successful:", res);
        const updateRes = await updateRecurringPaymentStatusById(id, false);
        if (!updateRes) {
        ...
        if (
            ...
            // nonce = nonce + 1;
            return await cancelRecurringPayment(id, nonce);
        } else {
            console.error("Error cancel recurring payment:", err);
            return { status: false, data: "" };
        }
    }
}
...
async function createUnsignedTransaction(
    ...
    const unsignedTx = await contract.cancelRecurringPayment.populateTransaction(id);
    //@ts-ignore
