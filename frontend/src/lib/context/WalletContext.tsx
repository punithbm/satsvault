"use client";

import React, {
  createContext,
  useState,
  useContext,
  ReactNode,
  useEffect,
} from "react";
import { toast } from "sonner";
import { Buffer } from "buffer";
import { AptosWalletAdapterProvider } from "@aptos-labs/wallet-adapter-react";
import { Network } from "@aptos-labs/ts-sdk";
import { OKXWallet } from "@okwallet/aptos-wallet-adapter";
import { MartianWallet } from "@martianwallet/aptos-wallet-adapter";
import Cookies from "js-cookie";
import Wallet, {
  AddressPurpose,
  request,
  Provider,
  getProviders,
} from "sats-connect";
import { isMobile } from "react-device-detect";

interface WalletContextType {
  walletAddress: string | null;
  btcAddress: string | null;
  isConnected: boolean;
  connect: any;
  disconnect: () => void;
  balance: number | null;
  getPublicKey: () => Promise<string | null>;
  addressType: string;
  setConnected: (account: any, btcAddress?: string) => void;
  checkCookieValidity: () => boolean;
}

const WalletContext = createContext<WalletContextType>({
  walletAddress: null,
  btcAddress: null,
  isConnected: false,
  connect: () => {},
  disconnect: () => {},
  balance: null,
  getPublicKey: async () => null,
  addressType: "",
  setConnected: (account: any, btcAddress?: string) => {},
  checkCookieValidity: () => true,
});

// Helper function to safely access localStorage
const getStorageValue = (key: string): string | null => {
  if (typeof window !== "undefined") {
    return localStorage.getItem(key);
  }
  return null;
};

// Helper function to safely set localStorage
const setStorageValue = (key: string, value: string): void => {
  if (typeof window !== "undefined") {
    localStorage.setItem(key, value);
  }
};

// Helper function to safely remove from localStorage
const removeStorageValue = (key: string): void => {
  if (typeof window !== "undefined") {
    localStorage.removeItem(key);
  }
};

export const WalletProvider: React.FC<{ children: ReactNode }> = ({
  children,
}) => {
  // Initialize state with null, then update after mount
  const [walletAddress, setWalletAddress] = useState<string | null>(null);
  const [btcAddress, setBtcAddress] = useState<string | null>(null);
  const [isConnected, setIsConnected] = useState(false);
  const [balance, setBalance] = useState<number | null>(null);
  const [addressType, setAddressType] = useState<string>("");
  const [publicKey, setPublicKey] = useState<string>("");

  // Check cookie on initial load
  useEffect(() => {
    const connectedCookie = Cookies.get("wallet_connected");
    if (connectedCookie === "true") {
      setIsConnected(true);
    }
  }, []);

  // Simple function to check if cookie is still valid
  const checkCookieValidity = () => {
    const connectedCookie = Cookies.get("wallet_connected");
    if (!connectedCookie && isConnected) {
      setWalletAddress(null);
      setBtcAddress(null);
      setIsConnected(false);
      setAddressType("");
      setBalance(null);
      removeStorageValue("walletAddress");
      removeStorageValue("btcAddress");
      removeStorageValue("porto_auth");
      toast.error("Session expired. Please connect your wallet again.");
      return false;
    }
    return true;
  };

  // Move localStorage check to useEffect
  useEffect(() => {
    const savedAddress = getStorageValue("walletAddress");
    const savedBtcAddress = getStorageValue("btcAddress");
    const savedAddressType = getStorageValue("addressType");
    setWalletAddress(savedAddress);
    setBtcAddress(savedBtcAddress);
    setAddressType(savedAddressType ?? "");
    setIsConnected(!!savedAddress);
  }, []);

  // Auto-reconnect on mount
  useEffect(() => {
    const autoConnect = async () => {
      try {
        //@ts-ignore
        const data = await request("getAccounts", {
          purposes: [AddressPurpose.Payment],
          message: "Auto-connect to SatsVault",
        });
        if (data.status === "success" && data.result) {
          const accounts = data.result;
          if (accounts.some((acc) => acc.address === walletAddress)) {
            setWalletAddress(walletAddress);
            setIsConnected(true);
          } else {
            // Stored address no longer valid
            removeStorageValue("walletAddress");
            setWalletAddress(null);
            setIsConnected(false);
          }
        }
      } catch (error) {
        console.error("Auto-connect failed:", error);
        removeStorageValue("walletAddress");
        setWalletAddress(null);
        setIsConnected(false);
      }
    };
    // autoConnect();
  }, [walletAddress]);

  const connect = async () => {
    try {
      if (isMobile && !window.XverseProviders) {
        window.open(
          "https://connect.xverse.app/browser?url=https://satsvault.com",
          "_blank",
          "noopener,noreferrer"
        );
        return;
      }
      Wallet.setCreateCustomConfig((providers) => {
        return {
          options: [
            {
              name: "Xverse",
              id: "XverseProviders.BitcoinProvider",
              icon: "data:image/svg+xml;base64,",
            },
            {
              name: "Unisat",
              id: "unisat",
              icon: "data:image/svg+xml;base64,",
            },
          ],
        };
      });
      //@ts-ignore
      const data = await Wallet.request("getAccounts", {
        purposes: [AddressPurpose.Payment],
        message: "Please connect to SatsVault",
      });

      if (data.status === "error" && data.error.code === -32603) {
        toast.error(
          "Wallets not found. Install Unisat or Xverse wallet to proceed further"
        );
        return;
      }
      if (data.status === "success") {
        const accounts = data.result;
        if (accounts.length > 0) {
          // setWalletAddress(accounts[0].address);
          // setAddressType(accounts[0].addressType);
          // setStorageValue("walletAddress", accounts[0].address);
          // setStorageValue("addressType", accounts[0].addressType);
          // const publicKey = data.result[0]?.publicKey;
          // setPublicKey(publicKey);
          // setIsConnected(true);
          // toast.success("Wallet connected");
          // Cookies.set("wallet_connected", "true", { expires: 7 });
        }
        return data;
      } else {
        toast.error("Failed to connect wallet");
      }
    } catch (error) {
      console.error("Connection failed:", error);
      toast.error("Failed to connect wallet");
    }
  };

  const setConnected = (account: any, btcAddress?: string) => {
    setWalletAddress(account.address);
    if (btcAddress) {
      setBtcAddress(btcAddress);
      setStorageValue("btcAddress", btcAddress);
    }
    setAddressType(account.addressType || "porto");
    setStorageValue("walletAddress", account.address);
    setStorageValue("addressType", account.addressType || "porto");
    const publicKey = account.publicKey;
    setPublicKey(publicKey);
    setIsConnected(true);
    toast.success("Wallet connected");
    Cookies.set("wallet_connected", "true", { expires: 7 });
  };

  const disconnect = async () => {
    try {
      // Remove Bitcoin wallet disconnect call to prevent sats-connect dialog
      // await Wallet?.disconnect(); // Removed
      setWalletAddress(null);
      setBtcAddress(null);
      setIsConnected(false);
      setAddressType("");
      setBalance(null);
      removeStorageValue("walletAddress");
      removeStorageValue("btcAddress");
      removeStorageValue("porto_auth");
      Cookies.remove("wallet_connected"); // Remove cookie on disconnect
      toast.success("Wallet disconnected");
    } catch (error) {
      console.error("Disconnection failed:", error);
      toast.error("Failed to disconnect wallet");
    }
  };

  const getPublicKey = async () => {
    if (!isConnected) {
      toast.error("Wallet not connected");
      return null;
    }

    try {
      return publicKey;
      // //@ts-ignore
      // const data = await Wallet.request("getAccounts", {
      //     purposes: [AddressPurpose.Payment],
      //     message: "Please connect to SatsVault"
      // });
      // if (data.status === "success" && data.result) {
      //     const publicKey = data.result[0]?.publicKey;
      //     if (publicKey) {
      //         return "02" + publicKey;
      //     } else {
      //         toast.error("Public key not found");
      //         return null;
      //     }
      // } else {
      //     toast.error("Failed to retrieve public key");
      //     return null;
      // }
    } catch (error) {
      console.error("Failed to get public key:", error);
      toast.error("Failed to retrieve public key");
      return null;
    }
  };

  const wallets = [new OKXWallet(), new MartianWallet()];

  return (
    <AptosWalletAdapterProvider
      dappConfig={{
        network: Network.TESTNET,
        aptosApiKey: "aptoslabs_NJoHD5Mx4ib_L6irxTUHFniM8mWBeFCtzgCSoUXcxR7WY",
      }}
      plugins={wallets}
      autoConnect
      onError={(error) => {}}
    >
      <WalletContext.Provider
        value={{
          walletAddress,
          btcAddress,
          isConnected,
          connect,
          disconnect,
          balance,
          getPublicKey,
          addressType,
          setConnected,
          checkCookieValidity,
        }}
      >
        {children}
      </WalletContext.Provider>
    </AptosWalletAdapterProvider>
  );
};

export const useBtcWallet = () => {
  const context = useContext(WalletContext);
  if (!context) {
    throw new Error("useWallet must be used within a WalletProvider");
  }
  return context;
};
